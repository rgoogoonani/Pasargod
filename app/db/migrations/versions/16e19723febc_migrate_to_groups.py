...
def upgrade() -> None:
    ...
    try:
        ...
        for row in result:
            ...
            if (
                len(users_dict[user_id]["excluded_inbounds"]) > 1
                and "No Excluded Inbounds" in users_dict[user_id]["excluded_inbounds"]
            ):
        ...
        for group_key, users in grouped.items():
            ...
            if dbinbounds:
                inbound_data = [{"inbound_id": i.id, "group_id": group_id} for i in dbinbounds if i.id is not None]

                # Add additional check to ensure we're not inserting empty values
                if inbound_data:
                    session.execute(inbounds_groups_association.insert(), inbound_data)
            ...
            for k, val in template_dict.items():
                if Counter(val["inbounds"]) == Counter(group_data["inbounds"]):
                    ...
                    if not exists:
                        ...
                        session.execute(
                            template_group_association.insert().values(user_template_id=int(k), group_id=group_id)
                        )
        ...
        if template_count > 0:
            ...
            for k, inbounds in grouped_data.items():
                group_name = f"group{counter}"
                template_groups = session.execute(
                    template_group_association.select().where(template_group_association.c.user_template_id == k)
                ).first()
                if not template_groups:
                    session.execute(group_table.insert().values(name=group_name))
                    group_id = session.execute(sa.select(group_table).where(group_table.c.name == group_name)).scalar()
